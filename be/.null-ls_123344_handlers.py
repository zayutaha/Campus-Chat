...
@app.post("/channels")
def create_channel(create_channel: CreateChannelRequest):
    channel_id = server.create_channel(create_channel.title)
    return {"channel_id": channel_id}

@app.get('/channels')
def list_channels():
    return { "channels": server.list_channels() }
...
def get_messages(channel_id: int):
    try:
        ...
        return [
            {
                "user_id": m.user_id,
                "user_name": server.get_user(m.user_id).name,
                "content": m.content,
            }
            for m in messages
        ]
                ...
                if client != ws:
                    try:
                        await client.send_text(
                            json.dumps({"user_id": user_id, "content": data, "user_name": user.name})
                        )
                    except Exception:
